  ...
  return (
    <div className="glass-panel rounded-3xl p-5 sm:p-6 border border-slate-200 flex flex-col justify-between h-full relative overflow-hidden">
      ...
      <div>
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-200">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-amber-600 font-bold uppercase tracking-wider">
          ...
          <div className="flex items-center gap-2">
            {isPrivileged && (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 text-xs font-mono flex items-center gap-1.5 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-climate-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>เพิ่มรายการ</span>
              </button>
            )}
            <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-rose-50 text-rose-700 border border-rose-200">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
          <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100">
            <span className="text-xs text-slate-500 block">เงินทุนรับสะสม</span>
          ...
          <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100">
            <span className="text-xs text-slate-500 block">ค่าใช้จ่ายจริง</span>
          ...
          <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100">
            <span className="text-xs text-slate-500 block">ยอดคงเหลือสุทธิ</span>
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-slate-900">{entry.entryTypeLabelTh}</span>
                      {isCorrection && (
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    {isPrivileged && !isCorrection && (
                      <button
                        ...
                        className="mt-1 text-[10px] font-mono text-cyan-600 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-climate-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white rounded"
                      >
                        + สร้างรายการแก้
                      </button>
                    )}
      ...
      <div className="mt-4 pt-3 border-t border-slate-200 text-[11px] text-slate-500 flex items-center justify-between">
        ...
        <span className="font-mono text-amber-600">Postgres Trigger Logged</span>
      </div>

      {/* Add Entry Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md rounded-3xl p-6 border border-amber-200 shadow-card-lg animate-in zoom-in-95 duration-150">
            <h3 className="text-sm font-bold text-slate-900 font-mono uppercase pb-2 border-b border-slate-200">
              เพิ่มรายการบัญชีใหม่ (Append-Only)
            </h3>

            <form onSubmit={handleCreateEntry} className="mt-4 space-y-3">
              <div>
                <label className="text-[11px] font-mono text-slate-500 block mb-1">ประเภทรายการ:</label>
                <select
                  value={entryType}
                  onChange={(e) => setEntryType(e.target.value as any)}
                  className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-900 focus:outline-none focus:border-climate-500 focus-visible:ring-2 focus-visible:ring-climate-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
                >
              ...
              <div>
                <label className="text-[11px] font-mono text-slate-500 block mb-1">จำนวนเงิน (บาท):</label>
                <input
                  type="number"
                  value={amount}
                  onChange={(e) => setAmount(e.target.value)}
                  className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono text-slate-900 focus:outline-none focus:border-climate-500 focus-visible:ring-2 focus-visible:ring-climate-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
                  required
                />
              </div>

              <div>
                <label className="text-[11px] font-mono text-slate-500 block mb-1">รายละเอียดรายการ:</label>
                <input
                  type="text"
                  value={description}
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs text-slate-500 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-climate-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
                >
                  ยกเลิก
                </button>
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-climate-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
                >
                  บันทึกลงบัญชี
                </button>
              </div>
      ...
      {showCorrectionModal && selectedTargetEntry && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md rounded-3xl p-6 border border-cyan-200 shadow-card-lg animate-in zoom-in-95 duration-150">
            ...
            <p className="text-xs text-slate-600 mt-2">
              ตามหลักการ Append-Only ข้อมูลเดิม <span className="font-mono font-bold text-cyan-700">({selectedTargetEntry.id})</span> จะไม่ถูกลบหรือแก้ไข แต่ระบบจะสร้างรายการชดเชยใหม่ที่เชื่อมโยงกลับไปยังรายการเดิม
            </p>

            <form onSubmit={handleCreateCorrection} className="mt-4 space-y-3">
              <div>
                <label className="text-[11px] font-mono text-slate-500 block mb-1">จำนวนเงินปรับปรุง (บาท):</label>
                <input
                  type="number"
              ...
              <div>
                <label className="text-[11px] font-mono text-slate-500 block mb-1">เหตุผลในการแก้ไข:</label>
                <input
                  type="text"
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-200">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-700 text-white font-bold text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-climate-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
                >
                  ยืนยันรายการแก้ไข
                </button>
              </div>
